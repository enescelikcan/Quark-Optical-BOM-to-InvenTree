...
class ImportWorker(QObject):
    ...
    def process_file(self, file_path: str, category) -> None:
        ...
        matched, missing = self.match_lines(lines)
        ...
        for line, part in matched:
            self.client.add_bom_item(assembly, part, line.quantity)

        self.log.emit(
            f"Imported {len(matched)} BOM item(s) into "
            f"'{assembly.name}' ({len(missing)} skipped)."
        )
    ...
    def match_lines(
        ...
        for line in lines:
            try:
                part = self.client.find_part_by_ipn(line.mpn)
    ...
    def resolve_assembly(self, project_name: str, category):
        ...
        if choice == ExistingProjectDialog.UPDATE:
            self.client.clear_bom(existing)
